...
export class StarTreeProvider implements vscode.TreeDataProvider<GistTreeItem> {
  ...
  async getChildren(element?: GistTreeItem): Promise<GistTreeItem[]> {
    try {
      if (!element) {
        return await this.getAllStarredGistItems();
      }
  ...
  private async getAllStarredGistItems(): Promise<GistTreeItem[]> {
    ...
    if (items.length === 0) {
      return [
        new EmptyNode(vscode.l10n.t('noStarredGists') || 'No starred gists'),
      ];
    }
  ...
  private async getFileItems(element: GistFolderNode): Promise<GistTreeItem[]> {
    ...
    const cacheKey = `starredGist:${element.providerId}:${element.gist.id}`;
    const cached = this.getCached<GistTreeItem[]>(cacheKey);
    if (cached) return cached;
